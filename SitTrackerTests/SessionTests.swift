...
final class SessionTests: XCTestCase {
    ...
    // MARK: - overlapping(start:stop:)

    func testOverlapping_noOverlap_before() {
        let base = Date()
        let existing = makeSession(start: base, minutes: 30, type: .supported)
        // Query range starts after existing ends
        XCTAssertTrue([existing].overlapping(start: base.addingTimeInterval(31 * 60), stop: base.addingTimeInterval(60 * 60)).isEmpty)
    }

    func testOverlapping_noOverlap_after() {
        let base = Date()
        let existing = makeSession(start: base.addingTimeInterval(60 * 60), minutes: 30, type: .supported)
        // Query range ends before existing starts
        XCTAssertTrue([existing].overlapping(start: base, stop: base.addingTimeInterval(59 * 60)).isEmpty)
    }

    func testOverlapping_partialOverlap_existingStartsBeforeRange() {
        let base = Date()
        // Existing: 0:00–1:00. Query: 0:30–1:30. Should overlap.
        let existing = makeSession(start: base, minutes: 60, type: .supported)
        let results = [existing].overlapping(start: base.addingTimeInterval(30 * 60), stop: base.addingTimeInterval(90 * 60))
        XCTAssertEqual(results.count, 1)
    }

    func testOverlapping_partialOverlap_existingEndsAfterRange() {
        let base = Date()
        // Existing: 0:30–1:30. Query: 0:00–1:00. Should overlap.
        let existing = makeSession(start: base.addingTimeInterval(30 * 60), minutes: 60, type: .supported)
        let results = [existing].overlapping(start: base, stop: base.addingTimeInterval(60 * 60))
        XCTAssertEqual(results.count, 1)
    }

    func testOverlapping_fullContainment_newInsideExisting() {
        let base = Date()
        // Existing: 0:00–2:00. Query: 0:30–1:30. Fully inside — should overlap.
        let existing = makeSession(start: base, minutes: 120, type: .supported)
        let results = [existing].overlapping(start: base.addingTimeInterval(30 * 60), stop: base.addingTimeInterval(90 * 60))
        XCTAssertEqual(results.count, 1)
    }

    func testOverlapping_fullContainment_existingInsideNew() {
        let base = Date()
        // Existing: 0:30–1:00. Query: 0:00–2:00. Existing fully inside query — should overlap.
        let existing = makeSession(start: base.addingTimeInterval(30 * 60), minutes: 30, type: .supported)
        let results = [existing].overlapping(start: base, stop: base.addingTimeInterval(120 * 60))
        XCTAssertEqual(results.count, 1)
    }

    func testOverlapping_activeSessions_excluded() {
        let base = Date()
        // Active session (no stopTime) should never be returned as an overlap.
        let active = Session(startTime: base, type: .supported)
        XCTAssertTrue([active].overlapping(start: base, stop: base.addingTimeInterval(60 * 60)).isEmpty)
    }

    func testOverlapping_adjacentAtBoundary_notOverlapping() {
        let base = Date()
        // Existing ends exactly when query starts — not an overlap (open interval).
        let existing = makeSession(start: base, minutes: 60, type: .supported)
        let queryStart = existing.stopTime!
        XCTAssertTrue([existing].overlapping(start: queryStart, stop: queryStart.addingTimeInterval(30 * 60)).isEmpty)
    }

    func testOverlapping_multipleOverlaps_allReturned() {
        let base = Date()
        let s1 = makeSession(start: base, minutes: 45, type: .supported)
        let s2 = makeSession(start: base.addingTimeInterval(30 * 60), minutes: 45, type: .supported)
        let s3 = makeSession(start: base.addingTimeInterval(120 * 60), minutes: 30, type: .supported) // outside range
        let results = [s1, s2, s3].overlapping(start: base, stop: base.addingTimeInterval(90 * 60))
